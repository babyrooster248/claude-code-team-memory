...
| Second member on ONE machine | Only if you cannot get a real second machine, and then: put `autoMemoryDirectory` in that clone's **`settings.json`**, not `settings.local.json` — the local file does not override it and the second agent will read the first member's notes. Check `~/.agent-knowledge-spool` is empty first: it is shared per machine, so a queued note is flushed with whichever credential runs next. Expect the "one Claude account, two credentials" warning unless you also switch Claude accounts |
