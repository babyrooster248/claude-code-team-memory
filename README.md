...
`AGENTS.md`, imported by `CLAUDE.md`, and deliberately small. The entry below is verbatim output
from a real end-to-end run — two Claude Code sessions on a sandbox project, no hand editing. The
header is the current template's, and it reads the way it does for a reason given below:
...
**Why the header addresses agents separately.** It used to open with *"Edit freely: delete a line you
disagree with"*, written for the reviewer looking at a pull request. Every teammate's agent reads this
file too, and the first measurement of the read path caught one acting on it: it hit the trap
firsthand, decided the entry deserved more confidence, and rewrote `(unconfirmed, 1 person)` to
`(confirmed, 2 people)` — a marker format that does not exist — while the `knowledge-state` block
below still recorded one contributor. The behaviour was reasonable. The permission was the mistake:
this file has two audiences and one of them will act on instructions written for the other. Counts
here come from verified credentials, so an agent that promotes an entry on its own recognisance
defeats the one rule the whole attribution design exists to protect.

Four things in that snippet are the whole design:
...
| An agent reading the artifact vs one without it, same task, 3 runs each | **−17.7% cost**, −26% turns, −22% wall — and every warm run cheaper than every cold one |
| Same comparison, avoiding the trap outright | **2 / 3** warm vs **0 / 3** cold |
